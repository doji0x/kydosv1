...
const chatTools=[...repositoryToolSchemas.filter(tool=>['listRepoTree','readFile','checkBranchStatus','listCommits','compareRefs','getCiLogs','inspectWriteAccess','commitFile'].includes(tool.function.name)),...referenceToolSchemas,...webToolSchemas,...auditToolSchemas];
const repo='doji0x/kydosv1';
export async function runManagerWithGithub({apiKey,model,messages,responseFormat,githubToken,base44,headSha,conversationId,log}){
