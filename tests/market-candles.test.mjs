...
test('samples produce honest OHLC with multiple observations in one minute and no synthetic volume', () => {
  ...
  const second = nextSampleCandle(initial, mint, '5m', { price: 12, blockId: 101 }, first + 30000);
  assert.equal(second.high, 12); assert.equal(second.close, 12); assert.equal(second.sample_count, 2);
  assert.equal(nextSampleCandle(second, mint, '5m', { price: 8, blockId: 99 }, first + 60000), null);
  assert.equal(nextSampleCandle(second, mint, '5m', { price: 8, blockId: 102 }, first + 30000), null);
});
