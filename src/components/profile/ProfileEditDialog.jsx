...
import { SOCIALS } from "@/components/profile/SocialLinks";
...
export default function ProfileEditDialog({ profile, open, onOpenChange }) {
  ...
  const [form, setForm] = useState({
    ...
    ...Object.fromEntries(SOCIALS.map((s) => [s.key, profile[s.key] || ""])),
  });
  const [saving, setSaving] = useState(false);
  const clean = (v) => v.trim().replace(/^@/, "").replace(/\s/g, "");
  const set = (k) => (e) =>
    setForm({ ...form, [k]: k === "handle" ? e.target.value.toLowerCase().replace(/[^a-z0-9_]/g, "").slice(0, 20) : k === "bio" ? e.target.value : clean(e.target.value) });
      ...
      <DialogContent className="bg-card sm:max-w-md">
        ...
        <form onSubmit={save} className="space-y-4">
          ...
          <div className="space-y-2">
            <Label className="text-xs uppercase tracking-wider text-muted-foreground">Social accounts</Label>
            {SOCIALS.map(({ key, label, icon: Icon }) => (
              <div key={key} className="relative">
                <Icon className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-muted-foreground" />
                <Input value={form[key]} onChange={set(key)} placeholder={`${label} username`} className="h-10 pl-9 bg-background font-mono text-sm" />
              </div>
            ))}
          </div>
