...
export const swapTopic = { uniswap_v2: [TOPIC.UNIV2_SWAP], uniswap_v3: [TOPIC.UNIV3_SWAP], uniswap_v4: [TOPIC.UNIV4_SWAP], kydos_curve: [TOPIC.KYDOS_BUY, TOPIC.KYDOS_SELL] };
export async function loadSwapPools(db, address, ethUsd) {
  ...
  return all.filter((p) => swapTopic[p.venue] && (!p.trust_status || p.trust_status === "TRUSTED")).map((p) => ({
    ...
    quote_usd: p.launchpad_verified ? ethUsd : isStable(p.quote_symbol) ? 1 : /^(WETH|ETH)$/i.test(p.quote_symbol || "") ? ethUsd : 0,
