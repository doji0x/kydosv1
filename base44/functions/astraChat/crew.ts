...
export async function runSpecialist({apiKey,chatModel,toolsModel,githubToken,role,job,context,log,beforeWrite}) {
 const spec=crewRoles[role]; if(!spec) throw new Error('Unknown specialist.'); const tools=spec.writes?toolSchemas.filter(x=>x.function.name==='commitFile'):[]; const model=tools.length?toolsModel:chatModel;
