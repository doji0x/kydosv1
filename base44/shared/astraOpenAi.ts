...
export async function callOpenAi({ apiKey, model, messages, tools, responseFormat }) {
  for (let attempt = 0; ; attempt++) {
    const response = await fetch('https://api.openai.com/v1/chat/completions', {
      ...
      body: JSON.stringify({ model, messages, ...(tools?.length ? { tools, tool_choice: 'auto', parallel_tool_calls: false, reasoning_effort: 'none' } : {}), ...(responseFormat ? { response_format: responseFormat } : {}) })
    });
