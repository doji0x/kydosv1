...
export default function TradeDrawer({ open, onOpenChange, trading, symbol }) {
  return <Drawer open={open} onOpenChange={value => { if (!trading.busy) onOpenChange(value); }}>
    <DrawerContent className="mx-auto max-h-[90vh] max-w-xl overflow-y-auto border-primary/25">
      ...
      {!trading.market ? <div className="mx-4 mb-8 rounded-xl border border-primary/20 bg-primary/10 p-4 text-sm"><p className="font-semibold text-primary">On-chain reserves unavailable</p><p className="mt-1 text-muted-foreground">Price and market cap remain live through Helius. Trading will unlock when the reserve snapshot is available.</p></div> :
