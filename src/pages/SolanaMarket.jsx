...
export default function SolanaMarket() {
  ...
  return <main className="mx-auto max-w-6xl space-y-5 px-4 py-6 pb-28">
    <MarketHeader mint={mint} market={trading.market}/>
    {trading.loadError && <p role="alert" className="rounded-xl border border-destructive/30 bg-destructive/10 p-4 text-sm text-destructive">{trading.loadError}</p>}
    ...
    <MarketChartSection feed={feed}/>
    {trading.market && <MarketStats market={trading.market} balances={trading.balances} stale={trading.stale}/>} 
    ...
    <div className="sticky bottom-20 z-30 flex gap-2 rounded-xl border border-primary/20 bg-background/90 p-3 shadow-2xl backdrop-blur-xl">
      ...
      <Button className="flex-1 gold-glow" disabled={!trading.market || trading.loading} onClick={openTrade}>Trade {trading.market?.symbol || 'token'}</Button>
    </div>
    <TradeDrawer open={tradeOpen} onOpenChange={setTradeOpen} trading={trading}/>
  </main>;
}