...
export default function MarketStats({ market, balances, stale, metrics }) {
  ...
  if (market) rows.push(['Curve tokens', formatAmount(market.tokenReserve, market.decimals)], ['SOL reserve', `${formatAmount(market.realSolReserve, 9)} SOL`], ['Curve status', market.graduated ? 'Graduated' : 'Bonding'], ['Snapshot', `${market.slot}${stale ? ' · stale' : ''}`]);
