...
export default function ProfileEditDialog({ profile, open, onOpenChange }) {
  const { refresh } = useMe();
  const [form, setForm] = useState({ handle: profile.handle || "", bio: profile.bio || "", avatar_url: profile.avatar_url || "" });
  const [saving, setSaving] = useState(false);
  const set = (k) => (e) => setForm({ ...form, [k]: k === "handle" ? e.target.value.toLowerCase().replace(/[^a-z0-9_]/g, "").slice(0, 20) : e.target.value });
