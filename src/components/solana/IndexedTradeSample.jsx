...
export default function IndexedTradeSample({ trades }) {
  if (!trades.length) return <p className="py-8 text-sm text-muted-foreground">No confirmed trades were parsed yet.</p>;
  return <div className="overflow-x-auto"><table className="w-full min-w-[640px] text-left font-mono text-xs">
    ...
    <tbody>{trades.map(trade => <tr key={trade.signature} className="border-b border-border/60">
      ...
      <td><a className="text-primary hover:underline" href={`https://solscan.io/tx/${trade.signature}`} target="_blank" rel="noreferrer">{trade.signature.slice(0, 8)}…</a></td>
    </tr>)}</tbody>
  </table></div>;
}