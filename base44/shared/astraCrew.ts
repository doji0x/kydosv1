...
export async function runSpecialist({apiKey,toolsModel,githubToken,base44,role,job,context,log,beforeWrite,recordAuditIssue,repoState,previousCheckpoint}){
 ...
 const tools=[...referenceToolSchemas,...readTools,...(spec.writes?repositoryToolSchemas.filter(x=>x.function.name==='commitFile'):[]),...(role==='integration'?repositoryToolSchemas.filter(x=>x.function.name==='mergeTaskBranch'):[]),...(role==='audit'?[auditTool]:[])];
