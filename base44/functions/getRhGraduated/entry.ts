// Public API: Kydos launches that filled their curve and graduated to a DEX pool.
...
export default async function (req: Request): Promise<Response> {
  try {
    ...
    const tokens = await db.entities.Token.filter({ status: "graduated" }, "-market_cap", limit);
