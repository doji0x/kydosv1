...
export async function runSpecialist({apiKey,toolsModel,githubToken,base44,role,job,context,log,beforeWrite,recordAuditIssue,repoState,previousCheckpoint}){
 ...
 const readTools=repositoryToolSchemas.filter(x=>['listRepoTree','readFile','createBranch'].includes(x.function.name));
 ...
 const tools=[...referenceToolSchemas,...readTools,...(spec.writes?repositoryToolSchemas.filter(x=>x.function.name==='commitFile'):[]),...(role==='audit'?[auditTool]:[])];
 ...
 const messages=[{role:'system',content:`You are Astra's ${spec.title}. ${spec.brief} The repository is doji0x/kydosv1 and the working branch is ${ASTRA_WORKING_BRANCH}. ${inspection} Continue only unfinished work from the last verified state. Re-read relevant files before editing. Never reset, pull, merge, recreate completed work, or overwrite unrelated changes. Treat main-branch changes from another agent as upstream evidence only; require its commit SHA, changed files, and handoff before depending on them. ${spec.writes?'Commit complete files only to astra/latest.':'You cannot write files.'} ${role==='audit'?'Call recordAuditIssue for every finding.':''} End with a concise implementation summary, checks run, and limitations.`},{role:'user',content:`JOB: ${job}\n\nCONTEXT:\n${context}`}];
 for(let step=1;step<=MAX_SPECIALIST_STEPS;step++){
  ...
  if(message.content)await log({kind:'reasoning',step,label:`Step ${step}`,detail:message.content.slice(0,ASTRA_LIMITS.event),nextStep:'Follow the specialist plan from this checkpoint.'});
  ...
  for(const call of message.tool_calls){const args=JSON.parse(call.function.arguments||'{}');if(!call.function.name.includes('Reference')&&call.function.name!=='recordAuditIssue')normalizeToolArgs(args);const start=Date.now();let result;
   try{if(call.function.name==='commitFile')await beforeWrite(args);result=call.function.name==='recordAuditIssue'?await recordAuditIssue(args):await runTool(githubToken,call.function.name,args,base44);}catch(error){result={error:error.message};}
