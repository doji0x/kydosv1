...
export default function ExternalMarket() {
  ...
  return <div className="mx-auto max-w-7xl px-4 pt-7 sm:px-6 sm:pt-9">
    ...
    <header className="mb-7 flex flex-wrap items-center justify-between gap-5"><div className="flex min-w-0 items-center gap-4"><MarketAvatar token={token} large/><div className="min-w-0"><div className="flex items-center gap-2"><h1 className="truncate font-display text-2xl font-semibold sm:text-3xl">{token.symbol}</h1>{token.verified && <BadgeCheck className="h-5 w-5 shrink-0 text-sky-400" aria-label="Listed as verified by Jupiter"/>}<WatchButton token={token} watchlist={watchlist}/></div><p className="text-sm text-muted-foreground">{token.name} · Solana mainnet</p><MintCopy mint={mint}/></div></div><a href={`https://jup.ag/tokens/${mint}`} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 rounded-lg bg-primary px-4 py-3 text-sm font-semibold text-primary-foreground">View on Jupiter<ArrowUpRight className="h-4 w-4"/></a></header>
    {watchlist.error && <p role="status" className="mb-4 text-xs text-amber-200">{watchlist.error}</p>}
    {!token.available && <p role="status" className="mb-5 rounded-xl border border-border bg-card px-4 py-3 text-sm text-muted-foreground">This coin has no market data in the current feed. Use its exact mint to inspect it on Jupiter or Solscan.</p>}
